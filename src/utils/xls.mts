...
// //  提取文件
export function extractFiles(path: string) {
  ...
  return new Promise((resolve, reject) => {
    const filePromises: Promise<any>[] = [] // 由于一份excel文档，会被解析成好多分xml文档，但是我们只需要两份xml文档，分别是（xl/sharedStrings.xml和xl/worksheets/sheet1.xml），所以用数组接受
    // stream
    //   .pipe(unzip.Parse())
    //   .on('error', reject)
    //   .on('close', () => {
